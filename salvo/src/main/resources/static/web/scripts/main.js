...
$('#sendShips').click(function(){
    var battleshipLocation = getShipLocation('#battleship');
    var patrol_boatLocation = getShipLocation('#patrol_boat');
...
//-----------------------------------------------------------------FUNCIONES
/**
 * Si el barco es vertical regresa true si es horizontal regresa false
 * @param {*} shipId 
 */
function isVertical(shipId) {
    return $(shipId).attr('data-gs-width') == 1 ? true : false;
}

function getVerticalLocation(coorX, coorY, space){
    let array = [];
    let letra = "";
    let numero = parseInt(coorX) + 1;
    for (let i = 0; i < space; i++) {
        letra = String.fromCharCode(65+ i + parseInt(coorY));
        array.push(letra+numero);
    }
    return array;
}

function getHorizontalLocation(coorX, coorY, space){
    let array = [];
    let letra = String.fromCharCode(65+ parseInt(coorY))
    let numero = 0;
    for (let i = 0; i < space; i++) {
        numero = parseInt(coorX) + 1 + i;
        array.push(letra+numero);
    }
    return array;
}

function getShipLocation(shipId) {
    let coorX = $(shipId).attr('data-gs-x');
    let coorY = $(shipId).attr('data-gs-y');
    let espacios = 0;
    let location = [];
    if(isVertical(shipId)){
        espacios = $(shipId).attr('data-gs-height');
        location = getVerticalLocation(coorX,coorY,espacios);
    }else{
        espacios = $(shipId).attr('data-gs-width');
        location = getHorizontalLocation(coorX,coorY,espacios);
    }
    return location;
}

function sendShipsPost(battleshipLocation, patrol_boatLocation, carrierLocation, submarineLocation, destroyerLocation){
    $.post({
      url: "/api/games/players/"+ getParamObj(window.location.search).gp +"/ships", 
      data: JSON.stringify([{ "type": "battleship",     "locations": battleshipLocation },
                            { "type": "patrol boat",    "locations": patrol_boatLocation },
                            { "type": "carrier",        "locations" : carrierLocation },
                            { "type": "submarine",      "locations" : submarineLocation },
                            { "type": "destroyer",      "locations" : destroyerLocation }]),
      dataType: "text",
